...
r.mean <- c( con_tcga$c.index,con_pcsi$c.index, con_pcsi$c.index,con_ouh$c.index,con_seq$estimate,con_micro$estimate,con_meta$estimate,NA, NA,
             con_tcga1$c.index,con_pcsi1$c.index, con_icgc_array1$c.index, con_ouh1$c.index, con_seq1$estimate, con_micro1$estimate, con_meta1$estimate)
           
r.lower <- c( con_tcga$lower,con_pcsi$lower, con_pcsi$lower,con_ouh$lower,con_seq_lower, con_micro_lower, con_meta_lower,NA, NA,
              con_tcga1$lower,con_pcsi1$lower, con_icgc_array1$lower, con_ouh1$lower, con_seq_lower1, con_micro_lower1, con_meta_lower1)
r.upper <- c( con_tcga$upper,con_pcsi$upper, con_pcsi$upper,con_ouh$upper,con_seq_upper, con_micro_upper, con_meta_upper, NA,NA, 
              con_tcga1$upper,con_pcsi1$upper, con_icgc_array1$upper, con_ouh1$upper, con_seq_upper1, con_micro_upper1, con_meta_upper1)
r.pval <- round(c(con_tcga$p.value, con_pcsi$p.value,con_icgc_array$p.value,con_ouh$p.value, con_seq_pval, con_micro_pval, con_meta_pval, NA, NA,
                  con_tcga1$p.value, con_pcsi1$p.value,con_icgc_array1$p.value,con_ouh1$p.value,  con_seq_pval1, con_micro_pval1, con_meta_pval1))
r.pval1 <- c(sprintf("%.1E", con_tcga$p.value),sprintf("%.1E", con_pcsi$p.value), sprintf("%.1E", con_icgc_array$p.value),
             sprintf("%.1E",con_ouh$p.value), sprintf("%.1E",con_seq_pval ),sprintf("%.1E", con_micro_pval), sprintf("%.1E", con_meta_pval),NA,NA,
             sprintf("%.1E", con_tcga1$p.value),sprintf("%.1E", con_pcsi1$p.value), sprintf("%.1E", con_icgc_array1$p.value),
             sprintf("%.1E",con_ouh1$p.value), sprintf("%.1E",con_seq_pval1 ),sprintf("%.1E", con_micro_pval1), sprintf("%.1E", con_meta_pval1))

t <- cbind(r.mean ,r.lower,r.upper,r.pval)
rownames(t) <-  c("TCGA","PCSI", "ICGC-array", "OUH","Sequencing","Micro-array","Overall","", 
                  "PCOSP","TCGA","PCSI", "ICGC-array", "OUH","Sequencing","Micro-array","Overall")
data2 <- 
  structure(list(
    mean  = c(NA,NA,t[,1]),
    lower = c(NA,NA,t[,2]),
...
r.mean <- c( log2(dindex_tcga$d.index), log2(dindex_pcsi$d.index), log2(dindex_icgc_array$d.index),log2(dindex_ouh$d.index),log2(dindex_seq$estimate),log2(dindex_micro$estimate),log2(dindex_meta$estimate),NA, NA,
             log2(dindex_tcga1$d.index),log2(dindex_pcsi1$d.index), log2(dindex_icgc_array1$d.index), log2(dindex_ouh1$d.index), log2(dindex_seq1$estimate), log2(dindex_micro1$estimate), log2(dindex_meta1$estimate))
r.lower <- c( log2(dindex_tcga$lower),log2(dindex_pcsi$lower), log2(dindex_icgc_array$lower),log2(dindex_ouh$lower),log2(dindex_seq_lower), log2(dindex_micro_lower), log2(dindex_meta_lower),NA, NA,
              log2(dindex_tcga1$lower),log2(dindex_pcsi1$lower), log2(dindex_icgc_array1$lower), log2(dindex_ouh1$lower), log2(dindex_seq_lower1), log2(dindex_micro_lower1), log2(dindex_meta_lower1))

r.upper <- c( log2(dindex_tcga$upper),log2(dindex_pcsi$upper), log2(dindex_icgc_array$upper),log2(dindex_ouh$upper),log2(dindex_seq_upper), log2(dindex_micro_upper), log2(dindex_meta_upper), NA,NA, 
              log2(dindex_tcga1$upper),log2(dindex_pcsi1$upper), log2(dindex_icgc_array1$upper), log2(dindex_ouh1$upper), log2(dindex_seq_upper1), log2(dindex_micro_upper1), log2(dindex_meta_upper1))

r.pval <- round(c(dindex_tcga$p.value, dindex_pcsi$p.value,dindex_icgc_array$p.value,dindex_ouh$p.value, dindex_seq_pval, dindex_micro_pval, dindex_meta_pval, NA, NA,
                  dindex_tcga1$p.value, dindex_pcsi1$p.value,dindex_icgc_array1$p.value,dindex_ouh1$p.value,  dindex_seq_pval1, dindex_micro_pval1, dindex_meta_pval1))

r.pval1 <- c(sprintf("%.1E", dindex_tcga$p.value),sprintf("%.1E", dindex_pcsi$p.value), sprintf("%.1E", dindex_icgc_array$p.value),sprintf("%.1E",dindex_ouh$p.value), sprintf("%.1E",dindex_seq_pval ),sprintf("%.1E", dindex_micro_pval), sprintf("%.1E", dindex_meta_pval),
             NA,NA,sprintf("%.1E", dindex_tcga1$p.value),sprintf("%.1E", dindex_pcsi1$p.value), sprintf("%.1E", dindex_icgc_array1$p.value),sprintf("%.1E",dindex_ouh1$p.value), sprintf("%.1E",dindex_seq_pval1 ),sprintf("%.1E", dindex_micro_pval1), 
             sprintf("%.1E", dindex_meta_pval1))



t <- cbind(r.mean ,r.lower,r.upper,r.pval)
rownames(t) <-  c("TCGA","PCSI", "ICGC-array", "OUH","Sequencing","Micro-array","Overall","", "PCOSP","TCGA","PCSI", "ICGC-array", "OUH","Sequencing","Micro-array","Overall")
