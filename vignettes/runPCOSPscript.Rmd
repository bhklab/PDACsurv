...
source(file.path("..", "R", "7_clinicalModelDindexCindex.R"))
...
source(file.path("..", "R", "8_existingClassifiersScoreCalculation.R"))

dataDir <- file.path("..", "data", "Classifier_Comparison")

# Birnbaum data

fileNames1 <- list.files(file.path(dataDir, "Birnbaum"))[-1]
geneCoefFile1 <-"bmc_genes.txt"
    
birnbaumScores <- parseGenefuFiles(file.path(dataDir, "Birnbaum"), fileNames1, geneCoefFile1)

# Chen data
fileNames2 <- list.files(file.path(dataDir, "Chen"))[-1]
geneCoefFile2 <- "chen_genes.txt"

chenScores <- parseGenefuFiles(file.path(dataDir, "Chen"), fileNames2, geneCoefFile2)

# Haider data (provided by author)
load(file.path("..", "data", "haiderScores.rda"))
