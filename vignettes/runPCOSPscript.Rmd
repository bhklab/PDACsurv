...
allValidationCohorts1 <- lapply(allValidationCohorts, function(cohort) as.data.frame(cohort, row.names=rownames(cohort)))

# Match names with cohortClasses and reorder cohorts to match between class and cohort data
names(allValidationCohorts1) <- tolower(names(allValidationCohorts1))
names(allValidationCohorts1)[which(names(allValidationCohorts1) == "icgc_seq")] <- "icgc"

# Remove ICGC_array_all
allValidationCohorts1 <- allValidationCohorts1[names(allValidationCohorts1) != "icgc_array_all"]
cohortClasses <- cohortClasses[names(cohortClasses) != "icgc_array_all"]

# Reorder validation cohrots to same order as cohortClasses
allValidationCohorts1 <- allValidationCohorts1[names(cohortClasses)]

# Get cohort survival data
allCohortSurvivalData <- lapply(allValidationCohorts1, 
                             function(cohort) {
                                 OS <- as.numeric.factor(cohort$OS)
                                 OS_Status <- as.numeric.factor(cohort$OS_Status)
                                 data.frame("OS"=OS, "OS_Status"=OS_Status,
...
allCohortProbabilites <- calculatePCOSPscores(allValidationCohorts1, selectedModels, nthread=15)
...
```{r forest_plot_subtype_comparison_dindex}
forestPlotCohortSubtypeComparison()
```


```{r forest_plot_subtype_comparison_cindex}
forestPlotCohortSubtypeComparison()
```
