...
chenSigScores <- readGenefuFilesToSigScores(file.path(classifierDir, "Chen"), fileNames2, geneCoefFile2)
...
# PCOSP data
PSCOSPscores <- validationStats$probabilities
...
cohortSurvivalData <- lapply(validationCohorts, 
                             function(cohort) {
                                 data <- cbind("OS"=cohort$OS, "OS_Status"=cohort$OS_Status)
                                 rownames(data) <- rownames(cohort)
                                 data})
...
metaestimateData <- list("birnbaum"=birnbaumSigScores, "chen"=chenSigScores, "haider"=haiderSigScores, 
