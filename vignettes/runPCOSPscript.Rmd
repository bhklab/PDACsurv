...
source(file.path("..", "R", "6_clinicalModelComparison.R"))
load(file.path(dataDir, "clinicalFeatures.rda"))
load(file.path(dataDir, "cohortClasses.rda"))

fitClinicalModels <- summarizeClinicalModels(clinicalFeatures, cohorts=c("icgc_clinical"))

ICGCclinicalModel <- fitClinicalModels$icgc_clinical
...
modelComparisonStats <- compareClinicalModels(clinicalFeatures, cohortClasses,
                                   models="icgc_clinical",
                      cohorts=c("pcsi", "tcga", "icgc_arr", "ouh"))

```

```{r barplot_model_comparison}
barplotModelComparison(modelComparisonStats, model="icgc_clinical",
                 names=c("TCGA","PCSI","ICGC-array","OUH"),
...
modelProbabilities <- calcualteCohortProbabilties(fitClinicalModels, clinicalFeatures, model="icgc_clinical")

modelDandCindex(modelProbablities, clinicalFeatures, model="icgc_clinical")
```
